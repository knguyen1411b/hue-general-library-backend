...
public class NotificationScheduler {
  ...
  /**
   * Cron chạy mỗi ngày lúc 00:00 - Kiểm tra những người sắp đến hạn mượn sách (dueDate trong 3 ngày
   * tới) Gửi thông báo nhắc nhở
   */
  ...
  public void sendDueDateReminders() {
    ...
    List<Rental> upcomingDueRentals =
        rentalRepository.findBorrowingRentalsWithDueDateBetween(today, threeDaysLater);
    ...
    for (Rental rental : upcomingDueRentals) {
      ...
      boolean alreadySentToday =
          notificationRepository
              .findByUserIdAndTypeOrderByCreatedAtDesc(
                  rental.getUserId(), NotificationType.RENTAL_DUE_REMINDER)
              .stream()
              .anyMatch(
                  n ->
      ...
      if (!alreadySentToday) {
        ...
        notificationService.createReminderNotification(
            ...
            "Bạn có phiếu mượn sách sẽ đến hạn trả vào ngày "
                + rental.getDueDate()
  ...
  /**
   * Cron chạy mỗi 7 ngày (thứ 2) - Gửi thông báo quá hạn cho những người trễ hạn Cấu hình: Chạy vào
   * 9:00 sáng thứ 2 hàng tuần
   */
  ...
  public void clearResolvedNotifications() {
    ...
    for (Notification notification : pendingReminders) {
      ...
      if (notification.getRelatedEntityId() != null
        ...
        Rental rental = rentalRepository.findById(notification.getRelatedEntityId()).orElse(null);
