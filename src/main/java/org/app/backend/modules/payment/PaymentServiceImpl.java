...
public class PaymentServiceImpl implements PaymentService {
  ...
  @Override
  public Page<PaymentDTO> findAll(Pageable pageable, UUID userId, PaymentType paymentType, PaymentStatus paymentStatus) {
    if (userId != null) {
      return paymentRepository.findByUserId(userId, pageable).map(p -> modelMapper.map(p, PaymentDTO.class));
    }
    if (paymentType != null) {
      return paymentRepository.findByPaymentType(paymentType, pageable).map(p -> modelMapper.map(p, PaymentDTO.class));
    }
    if (paymentStatus != null) {
      return paymentRepository.findByPaymentStatus(paymentStatus, pageable).map(p -> modelMapper.map(p, PaymentDTO.class));
    }
