...
public class UserSubscriptionServiceImpl implements UserSubscriptionService {
    ...
    public UserSubscriptionResponseDTO create(UserSubscriptionCreateDTO dto) {
        ...
        Subscription managedSubscription = subscriptionRepository
        ...
        var maybeActiveSubscription =
            userSubscriptionRepository.findActiveSubscriptionByUserId(
                managedUser.getId()
            );
        UserSubscription activeSubscription =
            maybeActiveSubscription != null
                ? maybeActiveSubscription.orElse(null)
                : null;

        if (activeSubscription != null) {
            LocalDate extendedEndDate = (
                activeSubscription.getEndDate().isAfter(today)
        ...
        LocalDate endDate = startDate.plusDays(
            managedSubscription.getDurationDays()
        );
    ...
    public List<UserSubscriptionResponseDTO> getAll() {
        return userSubscriptionRepository
            .findByStatusNot(UserSubscriptionStatus.DELETED)
            .stream()
        ...
        if (dto.getEndDate() != null) {
            ...
            if (userRenewRequest) {
                LocalDate minAllowedEndDate =
                    currentEndDate != null && currentEndDate.isAfter(today)
                        ? currentEndDate
                        : today;
    ...
    private UserSubscription getEntity(UUID id) {
        ...
        if (entity.getStatus() == UserSubscriptionStatus.DELETED) {
            throw new AppException(
                ...
                UserSubscriptionMessage.NOT_FOUND.getMessage()
            );
        }
    ...
    private void validateUserUpdatePermission(
        ...
        if (actor == null) {
            throw new AppException(
                ...
                UserSubscriptionMessage.AUTH_REQUIRED_UPDATE.getMessage()
            );
        }
        ...
        if (
            dto.getStatus() != null &&
            dto.getStatus() != UserSubscriptionStatus.CANCELED
        ) {
            throw new AppException(
                HttpStatus.FORBIDDEN,
                UserSubscriptionMessage.ACCESS_DENIED_UPDATE_STATUS.getMessage()
            );
        }
    ...
    private void markExpiredSubscriptions(UUID userId, LocalDate today) {
        ...
        if (
            expiredActiveSubscriptions == null ||
            expiredActiveSubscriptions.isEmpty()
        ) {
            return;
        }

        expiredActiveSubscriptions.forEach(subscription ->
            subscription.setStatus(UserSubscriptionStatus.EXPIRED)
        );
        userSubscriptionRepository.saveAll(expiredActiveSubscriptions);
    }

    private void markExpiredSubscription(
    ...
    private void validateUserSubscription(UserSubscription userSubscription) {
        if (userSubscription.getUser() == null) {
            throw new AppException(
                HttpStatus.BAD_REQUEST,
                UserSubscriptionMessage.VALIDATION_USER_REQUIRED.getMessage()
            );
        }
        ...
        if (
            ...
            throw new AppException(
                HttpStatus.BAD_REQUEST,
                UserSubscriptionMessage.END_DATE_BEFORE_START_DATE.getMessage()
            );
        }
    }
}
