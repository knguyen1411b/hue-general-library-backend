...
public class LibraryCardRequestServiceImpl implements LibraryCardRequestService {
  ...
  public LibraryCardRequestDTO getRequestById(UUID id) {
    LibraryCardRequest request =
        requestRepository
            .findWithUserById(id)
            .orElseThrow(
                () -> new AppException(HttpStatus.NOT_FOUND, "Không tìm thấy yêu cầu cấp thẻ"));
  ...
  @Override
  @Transactional
  public LibraryCardRequestDTO updateRequestStatus(
      UUID id, LibraryCardRequestStatus status, String note) {
    LibraryCardRequest request =
        requestRepository
            .findWithUserById(id)
            .orElseThrow(
    ...
    AuditLogAction action =
        status == LibraryCardRequestStatus.APPROVED
  ...
  public LibraryCardRequestDTO cancelRequest(UUID id, UUID userId) {
    ...
    if (request.getStatus() != LibraryCardRequestStatus.PENDING) {
      throw new AppException(
          HttpStatus.BAD_REQUEST, "Chỉ có thể hủy yêu cầu đang ở trạng thái chờ xử lý");
    }
  ...
  private void safeAuditLog(
    ...
    try {
      auditLogService.log(
          userId, username, action, AuditLogEntity.LIBRARY_CARD_REQUEST, entityId, status, message);
    } catch (Exception ex) {
